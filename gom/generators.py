...
def generate_knapsack(rng: random.Random, n: int = 20) -> OptimizationProblem:
    ...
    constraint = LinearConstraint("capacity", {f"x{i}": float(weights[i]) for i in range(n)}, "<=", float(capacity))
    return OptimizationProblem(id=f"kp-{rng.getrandbits(64):016x}", sense="max", variables=variables, objective=objective, constraints=[constraint], problem_type="knapsack", metadata={"weights": weights, "values": values, "capacity": capacity})
...
def generate_assignment(rng: random.Random, n: int = 6) -> OptimizationProblem:
    ...
    return OptimizationProblem(id=f"assign-{rng.getrandbits(64):016x}", sense="min", variables=variables, objective=objective, constraints=constraints, problem_type="assignment", metadata={"costs": costs})
